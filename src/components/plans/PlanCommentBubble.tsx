...
export const PlanCommentBubble: React.FC<PlanCommentBubbleProps> = ({
  ...
  const timeStr = new Date(comment.created_at).toLocaleTimeString('es-ES', {
    hour: '2-digit',
    minute: '2-digit',
  });
      ...
      {!isOwn && (
        <View style={styles.avatarWrapper}>
          {comment.sender?.avatarUrl ? (
            <Image source={{ uri: comment.sender.avatarUrl }} style={styles.avatar} />
          ) : (
      ...
      <View style={[styles.bubble, isOwn ? styles.bubbleOwn : styles.bubbleOther]}>
        {!isOwn && (
          <View style={styles.nameRow}>
            <Text style={styles.senderName}>{comment.sender?.name || 'Usuario'}</Text>
            {isCreator && (
