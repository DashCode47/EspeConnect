...
import { Plan, PlanParticipant, PlanParticipantRole, PlanParticipantStatus } from '../../domain/entities/plan.entity';
...
export const ManagePlanParticipantsScreen: React.FC = () => {
  ...
  const { fetchPlanById, approveParticipant, removeParticipant } = usePlanStore();
  ...
  const handleRemoveParticipant = (participant: PlanParticipant) => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          const success = await removeParticipant(planId, participant.userId);
          if (success) {
            setSuccessModal({
              visible: true,
  ...
  const handleApproveParticipant = async (participant: PlanParticipant) => {
    ...
    setActionLoading(participant.id);
    try {
      const success = await approveParticipant(planId, participant.userId);
      if (success) {
        setSuccessModal({
          visible: true,
  ...
  const allActive = participants.filter(p => p.leftAt === null);
  ...
  const availableSpots = plan?.maxParticipants
    ? Math.max(plan.maxParticipants - participantCount, 0)
    : null;
